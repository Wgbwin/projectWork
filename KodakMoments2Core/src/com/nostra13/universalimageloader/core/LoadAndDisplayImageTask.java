...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	private final ROIWithRotateDegree roiWithRotateDegree;
	...
	public LoadAndDisplayImageTask(ImageLoaderEngine engine,
			ImageLoadingInfo imageLoadingInfo, Handler handler,
			ROIWithRotateDegree roiWithRotateDegree) {
		this.engine = engine;
		this.imageLoadingInfo = imageLoadingInfo;
		this.handler = handler;
		this.roiWithRotateDegree = roiWithRotateDegree;
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		...
		try {
			...
			if (bitmap == null || bitmap.getWidth() <= 0
				...
				// TODO ImageLoader: Need check with Song. - Kane
				/*
				 * if (roi != null) { double ContainerW = bitmap.getWidth();
				 * double ContainerH = bitmap.getHeight(); int pX = (int) (roi.x
				 * * ContainerW); int pY = (int) (roi.y * ContainerH); int w =
				 * (int) (roi.w * ContainerW); int h = (int) (roi.h *
				 * ContainerH); bitmap = Bitmap.createBitmap(bitmap, pX, pY, w,
				 * h); }
				 */
				if (bitmap == null || bitmap.getWidth() <= 0
						|| bitmap.getHeight() <= 0) {
			...
			if (bitmap != null && roiWithRotateDegree != null) {
				double ContainerW = bitmap.getWidth();
				double ContainerH = bitmap.getHeight();
				double roiX = 0;
				double roiY = 0;
				double roiW = 0;
				double roiH = 0;
				int rotateDegree = roiWithRotateDegree.getRotateDegree();
				if (!ImageLoader.needChangePx2Py) {
					roiX = roiWithRotateDegree.getRoi().x;
					roiY = roiWithRotateDegree.getRoi().y;
					roiW = roiWithRotateDegree.getRoi().w;
					roiH = roiWithRotateDegree.getRoi().h;
				} else {
					roiX = roiWithRotateDegree.getRoi().y;
					roiY = roiWithRotateDegree.getRoi().x;
					roiW = roiWithRotateDegree.getRoi().h;
					roiH = roiWithRotateDegree.getRoi().w;
				}

				Matrix matrix = new Matrix();
				matrix.setRotate(rotateDegree, (float) (ContainerW / 2),
						(float) ContainerH / 2);
				Bitmap tempBitmap = Bitmap.createBitmap(bitmap, 0, 0,
						bitmap.getWidth(), bitmap.getHeight(), matrix, true);
				ContainerW = tempBitmap.getWidth();
				ContainerH = tempBitmap.getHeight();
				int pX = (int) (roiX * ContainerW);
				int pY = (int) (roiY * ContainerH);
				int w = (int) (roiW * ContainerW);
				int h = (int) (roiH * ContainerH);
				bitmap = Bitmap.createBitmap(tempBitmap, pX, pY, w, h);
			}
