...
public class ConnectionUtil {
	...
	/**
	 * check if connect printhub wifi
	 * @param context
	 * @return
	 */
	public static boolean isConnectedPrintHubWifi(Context context)
	{
		...
		if (myWifiInfo != null && info != null)
		{
			...
			returnValue = info.isConnected() && info.getType() == ConnectivityManager.TYPE_WIFI && isPrintHubWifi(ssid);
		}
		return returnValue;
	}
	...
	public static boolean isNearPrintHubWifi(Context context) {
		...
		//maybe it is not in scan list but is current connected kiosk (is this case exist?)
		WifiInfo info = wifiManager.getConnectionInfo();
		if (info != null && isPrintHubWifi(info.getSSID())) {
			return true;
		}
	...
	public static boolean isPrintHubWifi(String ssid) {
		if (ssid == null) {
			return false;
		}
		
		return ssid.toUpperCase(Locale.US).contains("KODAK-PRINT-PLACE");
	}
