...
public final class WifiConfigManager
{
	...
	private static void updateNetwork(WifiManager wifiManager, WifiConfiguration config)
	{
		...
		Log.i(TAG,"wifi config:"+config.toString());
		int networkId = wifiManager.addNetwork(config);
		if (networkId >= 0)
		{
//			if(config.SSID.contains("kodak")){
//				RssApp.getInstance().addWifiKiosk(networkId);
//			}
	...
	private static boolean isConnectKiosk(WifiManager wifiManager) {
		return wifiManager.getConnectionInfo() != null && ConnectionUtil.isKioskWifi(wifiManager.getConnectionInfo().getSSID());
	}
