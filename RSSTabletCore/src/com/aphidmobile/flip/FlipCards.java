...
public class FlipCards {
  ...
  public void reloadTexture(int frontIndex, View frontView, int backIndex, View backView) {
    synchronized (this) {
      boolean
          frontChanged =
          frontCards.loadView(frontIndex, frontView, controller.getAnimationBitmapFormat(),controller.getBackgroundDrawableForScreenShot(),controller.getLocation());
      boolean
          backChanged =
          backCards.loadView(backIndex, backView, controller.getAnimationBitmapFormat(),controller.getBackgroundDrawableForScreenShot(),controller.getLocation());
...
//          controller.getSurfaceView().requestRender();
//        }
    	  if(controller.isInFlipAnimation()){
    		  return false;
    	  }
    	  float delta = orientationVertical ? (lastPosition - event.getY()) : (lastPosition - event.getX());
    	  float absDelta = Math.abs(delta);
    	  if (absDelta > controller.getTouchSlop() && absDelta > MIN_MOVEMENT) {//ignore small movements
    		  ZoomStatus zs= controller.zoomStatus;
    		  int type = controller.getPhotoBookLayout().getType();
    		  if(delta > 0) {//gesture move left
    			  if(!zs.isZoomIn){
    				  autoFlipToNext();
    			  }else if(type == PhotoBookLayout.TYPE_SINGLE){
    				  autoFlipToNext();
    			  }else if(zs.leftPage){
    				  controller.getPhotoBookLayout().zoomInRight(true);
    			  }else{
    				  controller.getPhotoBookLayout().zoomInLeft(true);
    	  ...
    	  //Intercept Touch Event when flipping
    	  if(state != STATE_INIT){
    		  return true;
    	  }
    	  return isOnTouchEvent;
  ...
  public synchronized void autoFlipToNext(){
	  if(frontCards.getIndex() + 1 >= maxIndex){
		  return;
	  }
	  controller.showFlipAnimation();
	  forward = true;
  ...
  public synchronized void autoFlipToPrevious(){
	  if(frontCards.getIndex() -1 < 0){
		  return;
	  }
	  controller.showFlipAnimation();
	  forward = false;
	  ...
	  swapCards(); //frontCards becomes the backCards
      frontCards.resetWithIndex(backCards.getIndex() - 1);
      controller.flippedToView(getPageIndexFromAngle(accumulatedAngle), false);
	  setState(STATE_AUTO_ROTATE);
	  controller.getSurfaceView().requestRender();
  }
