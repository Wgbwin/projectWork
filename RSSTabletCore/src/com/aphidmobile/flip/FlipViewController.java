...
public class FlipViewController extends AdapterView<Adapter> {
  ...
  /**
 * calculate it from xml
 */
  private static final int PAGE_NUM_HEIGHT_DP = 28;
  /**
 * The minimum margin(dip) to left and right side for photobook when it is not zoom in
 */
  public static final int MARIN_LEFT_RIGHT_SIDE = 18;

  //photobook width and height will be init after parent.onSizeChanged()
  public double photoBookWidth = 1800;
  public double photoBookHeight = 1200;
  public int minHeight = 150;
  /** this value will maybe be convert to px when init*/
  public int marginSide = 20;
  private int pageNumTotalHeight;
  private PhotoBookLayout pbLayout;
  public int maxHeight;
  public int maxWidth;
  public ZoomStatus zoomStatus = new ZoomStatus();
  private WeakReference<Drawable> bgDrawableRef;
  private int[] location;
  private int[] locationWhenZoomInLeft;
  private int[] locationWhenZoomInRight;
  ...
  private void init(Context context, int orientation) {
	pageNumTotalHeight = DimensionUtil.dip2px(context, PAGE_NUM_HEIGHT_DP);
	Point screenSize = new Point();
	((Activity)context).getWindowManager().getDefaultDisplay().getSize(screenSize);
	
	//Note: this two value isn't exact here, they will be re-init when its parent onSizeChaned
  ...
  public void setPhotoBookLayout(PhotoBookLayout pblLayout){
	  this.pbLayout = pblLayout;
  }
  
  public PhotoBookLayout getPhotoBookLayout(){
	  return pbLayout;
  }
  
  public Drawable getBackgroundDrawableForScreenShot(){
	  if(bgDrawableRef == null || bgDrawableRef.get()==null){
		  PhotoBooksProductActivity activity = (PhotoBooksProductActivity) getContext();
		  bgDrawableRef = new WeakReference<Drawable>(activity.panel.getBackground());
	  }
	  Drawable drawable = bgDrawableRef.get();
	  return drawable;
  }
  
  public int[] getLocation(){
	  if(!zoomStatus.isZoomIn){
		  if(location == null){
			  location = new int[2];
			  getLocationInWindow(location);
		  }
		  return location;
	  }else if(zoomStatus.leftPage){
		  if(locationWhenZoomInLeft == null){
			  locationWhenZoomInLeft = new int[2];
			  getLocationInWindow(locationWhenZoomInLeft);
		  }
		  return locationWhenZoomInLeft;
	  }else{
		  if(locationWhenZoomInRight == null){
			  locationWhenZoomInRight = new int[2];
			  getLocationInWindow(locationWhenZoomInRight);
		  }
		  return locationWhenZoomInRight;
	  }
  }
   ...
   * 
   * Be careful, only ARGB_8888 is work correct, the others may cause screenShot bugs(image distorted), added by Robin
  ...
  public void flipToNext(){
	  cards.autoFlipToNext();
  }
  
  public void flipToPrevious(){
	  cards.autoFlipToPrevious();
  }
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
	...
	int width = getLayoutWidthByHeight(height);
	if(!zoomStatus.isZoomIn){
		if(width>maxWidth-marginSide*2){
			width = maxWidth-marginSide*2;
			height = getLayoutHeightByWidth(width);
		}
		
		if(height>maxHeight){
			height = maxHeight;
			width = getLayoutWidthByHeight(height);
		}
	}
    ...
    AphidLog.d("OnMeasure: %d, %d", widthMeasureSpec,heightMeasureSpec);
    for (View child : bufferedViews) {
  ...
  public void flippedToView(final int fromIndex,final int toIndex,  boolean isPost) {
    if (AphidLog.ENABLE_DEBUG) {
      AphidLog.d("flippedToView: %d, isPost %s", toIndex, isPost);
    }

    debugBufferedViews();

    if (toIndex >= 0 && toIndex < adapterDataCount) {
      if (toIndex == adapterIndex + 1) { //forward one page
        if (adapterIndex < adapterDataCount - 1) {
          adapterIndex++;
          View old = bufferedViews.get(bufferIndex);
          if (bufferIndex + 1 > sideBufferSize) {
          ...
          bufferIndex = bufferedViews.indexOf(old) + 1;
          requestLayout();
          //TODO in github, someone said this will cause flash issue, so comment it. I'm not sure it can fix the flash problem.
//          updateVisibleView(bufferIndex);
        }
      } else if (toIndex == adapterIndex - 1) {
        if (adapterIndex > 0) {
          adapterIndex--;
          View old = bufferedViews.get(bufferIndex);
          if (bufferIndex - 1 + sideBufferSize < bufferedViews.size() - 1) {
...
//          updateVisibleView(bufferIndex);
      ...
      Assert.fail("Invalid indexInAdapter: " + toIndex);
    }
    //debugBufferedViews();
  }
  ...
  private void hideFlipAnimation() {
    if (inFlipAnimation) {
    	...
    	if(adapter instanceof PhotoBooksProductMainAdapter){
    		PhotoBooksProductMainAdapter pma = (PhotoBooksProductMainAdapter) adapter;
    		if(pma.isNeedNotifyAfterFlipped){
  ...
  public int[] getPageMaxSizeWhenNotZoomIn(){
	  return new int[]{maxWidth/2,getLayoutHeightByWidth(maxWidth)-pageNumTotalHeight};
  }
  
  public int getLayoutWidthWhenZoomIn(){
		return getLayoutWidthByHeight(maxHeight);
  }
  
  public int getLayoutWidthByHeight(int height){
		int imgHeight = height - pageNumTotalHeight;
		return (int) (imgHeight * photoBookWidth*2.0/photoBookHeight);
	}
	
  public int getLayoutHeightByWidth(int width){
		return (int) (width * photoBookHeight/2.0/photoBookWidth) + pageNumTotalHeight;
  }
  
  public static class ZoomStatus{
	public boolean isZoomIn = false;
	public boolean leftPage = false;
  }
}