...
public class FacebookFriendsAdapter extends CanZoomBaseAdapter implements onProcessImageResponseListener{	
	private LayoutInflater mInflater;		
	ArrayList<FbkObject> fbkUserList;
	Bitmap picturestack;		
	private int size ;			
		
	public FacebookFriendsAdapter(Context context,ArrayList<FbkObject> fbkUsers,LruCache<String, Bitmap> mMemoryCache) {
		super(context,mMemoryCache);			
		this.fbkUserList = fbkUsers;
		size =fbkUserList.size();		
	...
	private Bitmap getBitmap(FbkUser user,int position,ImageView imageView) {	
		Bitmap result = null;
		...
		if (position == 0) {			
			...
			URI pictureURI = user.getUserUri();
			if (pictureURI != null) {					
	...
	public View getView(int position, View convertView, ViewGroup parent) {							
		...
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.grid_facebook_friend_item, null);	
			holder = new FacebookFriendsHolder();
			...
			holder = (FacebookFriendsHolder) convertView.getTag();
		}
		
		FbkUser user = (FbkUser) fbkUserList.get(position);		
		holder.photoName.setText(user.name);	
		
		Bitmap bitmap = getBitmap(user, position,holder.imageView);
	...
	public void cancelRequest(){		
		if (fbkUserList == null) return; 		
		int size = fbkUserList.size();
		if (size == 0) return; 
		for (int i = size - 1; i >= 0; i--) {
			FbkUser user = (FbkUser) fbkUserList.get(i);
			if (user == null) continue; 
			cancelServerRequest(user.ID);
		}
	}	
	
	class FacebookFriendsHolder {		
		TextView photoName;		
		ImageView imageView;
	}
