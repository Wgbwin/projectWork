...
/*
 * This extension of SquareImageView can be used to allow a
 * user to manually adjust a ROI.
 */
public class ROIImageView extends SquareImageView {
	...
	/**
	 * This class assumes the image in the ImageView is shown fitCenter
	 * and is not square
	 * 
	 */
	public ROIImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mNinePatch = (NinePatchDrawable) getContext().getResources()
				.getDrawable(R.drawable.highlight_change);

		mGlobalListener = new OnGlobalLayoutListener() {

			@Override
			public void onGlobalLayout() {
				BaseActivity.removeViewTreeObserverVersionSafe(this,
						ROIImageView.this);
				...
				mImageDrawableRec = new RectF(imageDrawableX, imageDrawableY,
						imageDrawableX + imageDrawableWidth, imageDrawableY
								+ imageDrawableHeight);
				if (mInitialROI == null) {
					...
					if (mRect == null) {
						...
						// Initializing rect
						// TODO face detection. This can't be done right here
						...
						float startX = mImageDrawableRec.left
								+ (mImageDrawableRec.width() - mCurWidth)
								* 0.5f;
						float startY;
						if (mImageDrawableRec.width() > mImageDrawableRec
								.height()) {
							Log.w(TAG, "Image is landscape");
							startY = mImageDrawableRec.top
									+ (mImageDrawableRec.height() - mCurHeight)
									* 0.5f;
						} else {
						...
						// setting
						// the fields;
						mRect = new RectF(startX, startY, startX + mCurWidth,
								startY + mCurHeight);
					...
					Log.e(TAG,
							"mRectF mInitialROI:" + mInitialROI.toShortString());
					Log.e(TAG,
							"mRectF AftermInitialROI:" + mRect.toShortString());
					mInitialROI = null;
	...
	public RectF getROI() {
		...
		float imageX = v[Matrix.MTRANS_X];
		float imageY = v[Matrix.MTRANS_Y];
		float tempX = ((mRect.left - imageX - getPaddingLeft()) / mImageDrawableRec
				.width());
		float tempY = ((mRect.top - imageY - getPaddingTop()) / mImageDrawableRec
				.height());
	...
	public void checkBounds() {
		...
		// Log.i(TAG, "ImageCoords mRect.left:" + mRect.left +
		// " mImageDrawableRec.left:" + mImageDrawableRec.left + " mRect.top:" +
		// mRect.top + " mImageDrawableRec.top:" + mImageDrawableRec.top);

		if ((int) mRect.left < (int) mImageDrawableRec.left
				|| (int) mRect.top < (int) mImageDrawableRec.top) {
			Log.e(TAG,
					"Very odd error, they must have changed the screen size... Reset to center square");
			float minDim = (int) Math.min(mImageDrawableRec.width(),
					mImageDrawableRec.height());
			mRect.left = mImageDrawableRec.left
					+ (mImageDrawableRec.width() - minDim) / 2;
			mRect.right = mRect.left + minDim;
			mRect.top = mImageDrawableRec.top
					+ (mImageDrawableRec.height() - minDim) / 2;
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (true) {
			//Currently ROI can not be manualy set.
			//So just ignore touch events.
			return false;
		}
		...
		if (numPointers == 2) {// zoom
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (mCurHeight > Math.min(mImageDrawableRec.width(),
						mImageDrawableRec.height())) {
					mCurHeight = Math.min(mImageDrawableRec.width(),
							mImageDrawableRec.height());
				}
				if (mCurWidth > Math.min(mImageDrawableRec.width(),
						mImageDrawableRec.height())) {
					mCurWidth = Math.min(mImageDrawableRec.width(),
							mImageDrawableRec.height());
				}
	...
	protected void onDraw(Canvas canvas) {
		...
		//Current manual ROI is not used,
		//so don't draw the graphic.
