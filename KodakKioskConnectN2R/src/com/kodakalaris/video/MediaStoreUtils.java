...
/**
 * This class contains an assortment of public static methods which 
 * are used by various classes.
 * 
 * These have to do with android's MediaStore, as well as
 * Bitmap Compressing, and exif data reading and parsing.
 */
public class MediaStoreUtils {
	...
	public static Bitmap getFullRes(Context context, String filePath, int width, int height, int resolution) {
		...
		if (resolution == SquareImageView.RESOLUTION_HIGH) {
			o.inSampleSize = calculateInSampleSize(filePath, width / 2, height / 2);
	...
	public static String getExifTimeDateUserFriendly(String filePath) {
		...
		try {
			...
			if (dateTime != null) {
				...
				// Update the date to Locale format
				...
				if (split.length > 0) {
					for(int i=0; i<split.length; i++){
						...
						for(int j=0; temp!=null && j<temp.length; j++){
							timeFields.add(temp[j]);
						}
					}
					//dateTime = split[0].replace(':', '-') + " " + split[1];
				}
				Calendar calendar = Calendar.getInstance();
				calendar.set(Calendar.MONTH, Integer.parseInt(timeFields.get(1))-1);
				dateTime = calendar.getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.getDefault()) + " " + timeFields.get(2) + ", " + timeFields.get(0);
			}
	...
	public static String getExifTimeDateStoryDocFormat(String filePath) {
		if (!new File(filePath).exists()) {
			return "";
		}
		ExifInterface exif;
