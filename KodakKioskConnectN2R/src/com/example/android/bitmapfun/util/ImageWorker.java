...
public abstract class ImageWorker {
     ...
     * @param imageView The ImageView to bind the downloaded image to.
     */
    public void loadImage(int groupPosition, int childPosition, ImageView imageView) {
        if (mImageWorkerAdapter != null) {
            loadImage(mImageWorkerAdapter.getItem(groupPosition,childPosition), imageView);
        } else {
    ...
    private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(Object... params) {
        	data = params[0];
        	ProductInfo tempItem = (ProductInfo)data;
        	if(tempItem.productType.equals(AppConstants.BOOK_TYPE)){
        		//changed by song
        		Photobook photobook = PhotobookUtil.getPhotobookFromList(tempItem.ProductId);
        		PhotoBookPage page = photobook.photoBookPages.get(2);
        		Bitmap bitmapQuickBook = null;
        		ThumbnailProvider mProvider = ThumbnailProvider.obtainInstance(mContext);
        		bitmapQuickBook = mProvider.getMini(page.sPhotoBookPageID);
        		if(null == bitmapQuickBook){
        			byte[] imgRawData = startDownload(page.sPhotoBookPageURL);
                    try {
    					mProvider.cacheMini(page.sPhotoBookPageID, imgRawData);
    				} catch (Exception e) {
    					// TODO Auto-generated catch block
    					e.printStackTrace();
    				}
                    if(imgRawData!=null){
                    	bitmapQuickBook = BitmapFactory.decodeByteArray(imgRawData, 0, imgRawData.length);
                    }
        		}
                
        		return bitmapQuickBook;
        	}
            //final String dataString = String.valueOf(tempItem.photoInfo.getLocalUri());          
            Bitmap bitmap = null;
            final String fileName = String.valueOf(tempItem.photoInfo.getPhotoPath());
            ...
            /*if (mImageCache != null && !isCancelled() && getAttachedImageView() != null
                    && !mExitTasksEarly) {
                bitmap = mImageCache.getBitmapFromDiskCache(fileName);
            }*/
    ...
    private void setImageBitmap(ImageView imageView, Bitmap bitmap) {
        if (mFadeInBitmap) {
            ...
            imageView.setImageBitmap(
                    new BitmapDrawable(mContext.getResources(), mLoadingBitmap).getBitmap());
    ...
    public static abstract class ImageWorkerAdapter {
        public abstract Object getItem(int groupPosition,int childPosition);
        public abstract int getSize();
    }
    
    private byte[] startDownload(String reqUrl) {
