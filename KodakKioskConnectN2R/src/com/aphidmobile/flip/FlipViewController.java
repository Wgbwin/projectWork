...
public class FlipViewController extends AdapterView<Adapter> {
	...
	private boolean blockOutsideTouch = false;
	...
	// this part only for QuickBook
	public boolean hasPageSelected = false;
	private CropView currentView;
	public CropView getCurrentView() {
		return currentView;
	}

	public void setCurrentView(CropView currentView) {
	...
	public boolean onTouchEvent(MotionEvent event) {
		if(QuickBookFlipperActivity.pageUploading || blockOutsideTouch){
			return true;
		}
	...
	public void destroyAdapter(){
		if(bufferedViews!=null && bufferedViews.size()>0){
			/*for(View view : bufferedViews){
				bufferedViews.remove(view);
				view = null;
			}*/
			bufferedViews.clear();
		}
	...
	void flippedToView(int indexInAdapter) {
		//XXX: can be simplified and unified with setSelection
		if (AphidLog.ENABLE_DEBUG)
			AphidLog.d("flippedToView: %d", indexInAdapter);
		if(currentView!=null && currentView.mIsInEditMode){
			Log.e("FlipViewController", "currentView is not null.");
			return;
		}
		debugBufferedViews();

		if (indexInAdapter >= 0 && adapter!=null && indexInAdapter < adapter.getCount()) {
		...
		debugBufferedViews();
		PrintHelper.flipIndex = indexInAdapter;
	}

	void postHideFlipAnimation(final boolean visible) {
		handler.post(new Runnable() {
			@Override
			public void run() {
				hideFlipAnimation(visible);
			}
		});
	}
	...
	void showFlipAnimation() {
		if (!inFlipAnimation) {
			...
			PrintHelper.isFlippingAnimation = inFlipAnimation;
	...
	private void hideFlipAnimation(final boolean visible) {
		if (inFlipAnimation) {
	...
	public boolean isBlockOutsideTouch() {
		return blockOutsideTouch;
	}



	public void setBlockOutsideTouch(boolean blockOutsideTouch) {
		this.blockOutsideTouch = blockOutsideTouch;
	}



	public void autoFlipAnimation(final int index) {
		//postFlippedToView(index);
		getCards().showAutoFlip(index);
	}
}