...
public class EditTestInputDialog extends Dialog
{
	 ...
	 *            The theme id.
	 */
	public EditTestInputDialog(Context context, int theme)
	{
		super(context, theme);
	}
	...
	public static class EditTestInputDialogBuilder
	{
		 ...
		 *            The context.
		 */
		public EditTestInputDialogBuilder(Context context,int maxLines,GreetingCardPageLayer layer)
		{
		...
		{	
			...
			final EditTestInputDialog dialog = new EditTestInputDialog(context, R.style.Dialog);
			View layout = inflater.inflate(R.layout.greetingcardedittextinput, null);
			editTextInput = ((EditText) layout.findViewById(R.id.editTextInput));
			showSoftInput();
			...
			//final EditText editInput = (EditText) layout.findViewById(R.id.editTextInput);
			...
			if(!message.equals("") && !message.equals(layer.getTextInputDefaultValue())){
				editTextInput.setText(message);
				editTextInput.setCursorVisible(true);
				editTextInput.setSelection(message.length());
			}else {
				editTextInput.setHint(layer.getTextInputDefaultValue());
				editTextInput.setHighlightColor(Color.BLACK);
				editTextInput.setCursorVisible(false);
				editTextInput.setOnClickListener(new View.OnClickListener() {							
					@Override
					public void onClick(View v) {
						editTextInput.setCursorVisible(true);
						if (ifFistTimeClick && editTextInput.getText().length() ==0){
							editTextInput.setCursorVisible(true);
							editTextInput.setHint("");
							editTextInput.setText(layer.getTextInputDefaultValue());
							editTextInput.setSelection(layer.getTextInputDefaultValue().length());
							ifFistTimeClick = false;
						}
					}
				});
			}
