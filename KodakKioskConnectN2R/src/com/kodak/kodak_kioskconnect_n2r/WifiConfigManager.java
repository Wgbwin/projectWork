...
public final class WifiConfigManager
{
	...
	// WifiConfigManager.class.getSimpleName();
	 ...
	 */
	public static String WIFI_CONNECT_SUCCESS = "Success";
	public static String WIFI_CONNECT_FAILED = "Failed";
	private static void updateNetwork(WifiManager wifiManager, WifiConfiguration config)
	{
		Integer foundNetworkID = findNetworkInExistingConfig(wifiManager, config.SSID);
		if (foundNetworkID != null)
		{
			PrintHelper.status = "Removing old configuration for network " + config.SSID;
		...
		if (networkId >= 0)
		{
			...
			if(Connection.isKioskWifi(config.SSID)){
			...
			{
				PrintHelper.status = "" + config.SSID;
				wifiManager.saveConfiguration();
				...
				if (isConnectKiosk(wifiManager)) {
					PrintHelper.networkStatus = WIFI_CONNECT_SUCCESS;
				} else {
					PrintHelper.status = "Failed to enable network " + config.SSID;
					PrintHelper.networkStatus = WIFI_CONNECT_FAILED;
				}
			}
			else
			{
		...
		{
			PrintHelper.status = "Unable to add network " + config.SSID;
			PrintHelper.networkStatus = WIFI_CONNECT_FAILED;
		}
	}
