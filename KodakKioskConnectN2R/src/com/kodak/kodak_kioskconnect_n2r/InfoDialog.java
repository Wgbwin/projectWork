...
public class InfoDialog extends Dialog {
	...
	public static class InfoDialogBuilder {
		...
		public InfoDialog create() {
			...
			/**
			 * fixed for RSSMOBILEPDC-1746
			 * The message pop takes too much place on portrait view.
			 * add by song
			 */
			if (!AppContext.getApplication().isScreenOrientationFlag()){
				LinearLayout LinearLayout02 = (LinearLayout) layout.findViewById(R.id.LinearLayout02);
				LinearLayout.LayoutParams mLayoutParams = new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, 0, 2);
				LinearLayout02.setLayoutParams(mLayoutParams);
			}
			...
			if (PrintHelper.isRecommendWiFiDialog) {
				// negativeButton.setBackgroundColor(R.drawable.next_button);
				negativeButton.setBackgroundResource(R.drawable.next_button);
				PrintHelper.isRecommendWiFiDialog = false; // when the backround
			}
			// set the dialog title
			titleTV.setText(title);
			titleTV.setTypeface(PrintHelper.tf);
			messageTV.setTypeface(PrintHelper.tf);
			positiveButton.setTypeface(PrintHelper.tf);
			negativeButton.setTypeface(PrintHelper.tf);
